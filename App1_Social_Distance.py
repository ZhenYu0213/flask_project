...
def cvDrawBoxes(detections, img):
    ...
    # =================================================================
        ...
        red_zone_list = []
        red_line_list = []
        # Get all the combinations of close detections, #List of multiple items - id1 1, points 2, 1,3
        for (id1, p1), (id2, p2) in combinations(centroid_dict.items(), 2):
            # Check the difference between centroid x: 0, y :1
            dx, dy = p1[0] - p2[0], p1[1] - p2[1]
            distance = is_close(dx, dy) 			# Calculates the Euclidean distance
            # Set our social distance threshold - If they meet this condition then..
            if distance < 75.0:
        ...
        for idx, box in centroid_dict.items():  # dict (1(key):red(value), 2 blue)  idx - key  box - value
            if idx in red_zone_list:   # if id is in red zone list
                # Create Red bounding boxes  #starting point, ending point size of 2
                cv2.rectangle(img, (box[2], box[3]),
                              (box[4], box[5]), (255, 0, 0), 2)
            else:
                # Create Green bounding boxes
                cv2.rectangle(img, (box[2], box[3]),
        ...
        for check in range(0, len(red_line_list)-1):
            ...
            if (check_line_x < 75) and (check_line_y < 25):
                # Only above the threshold lines are displayed.
